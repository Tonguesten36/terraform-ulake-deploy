...
resource "azurerm_container_app" "folder_service" {
  ...
  template {
    container {
      name  = "ulake-service-folder"
      image = "${azurerm_container_registry.acr.login_server}/ulake-service-folder:latest"
  ...
  depends_on = [
    ...
    azurerm_container_app.acl_service
  ]
}
