...
resource "azurerm_container_app" "dashboard_service" {
  ...
  template {
    container {
      name  = "ulake-service-dashboard"
      image = "${azurerm_container_registry.acr.login_server}/ulake-service-dashboard:latest"
  ...
  depends_on = [
    ...
    azurerm_container_app.folder_service,
    azurerm_container_app.core_service
  ]
}
